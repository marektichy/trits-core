...
class TritsGame {
    ...
    constructor(address) {
        ...
        this.status = -1; // Status: -1 => waiting to be initialized, 0 => running, 1 => finished
        this.flip_treshold = TRITS_GAME_FLIP_TRESHOLD;
        this.nominal = 0; // Nominal value of the game
        this.triangle = new TritsTriangle(0, 0, 0);
        this.bonus = 0; // The amount of bonus coins
        this.last_player = 0; // sender ID of the last player so far
    }
    ...
    putBonus(bonus) {
        ...
        if (this.status != 0) { // Only running games (with a set nominal) can accept bonus from the bank
            return 0
        }

        this.bonus = bonus;
        return -(this.nominal * bonus);
    }
    ...
    putBet(amount, sender) {
        ...
        // First bet starts the game, sets the nominal and the created and changed timestamps
        if (this.status == -1) {
            var now = moment();
            this.created = now.format('x');
     ...
     */
    /* Complex calculation of the rewards is obsolete now, the winner takes it all (except for the game initiator)
    getRewardsTable() {
        ...
        if (winning_side == 'G') {
            ...
            // Occasionally there is not enough money
    ...
    */
    ...
    /**
     * @method getSinceLastUpdate
     * Return the number of second since the last game status change or -1 if it hasn't started yet
     */
